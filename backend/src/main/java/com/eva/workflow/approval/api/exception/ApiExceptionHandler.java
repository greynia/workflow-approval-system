...
public class ApiExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleApplicationException(ApplicationException exception, HttpServletRequest request) {
        ...
        return ResponseEntity.status(status)
                .body(new ErrorResponse(exception.getCode(), messageResolver.resolveApplicationMessage(exception), requestId(request)));
    }
    ...
    public ResponseEntity<ErrorResponse> handleMessageNotReadable(HttpMessageNotReadableException exception, HttpServletRequest request) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(new ErrorResponse(
                        ...
                        messageResolver.resolve("error.INVALID_REQUEST_BODY", null, "Request body is missing or malformed."),
                        requestId(request)
                ));
    }
    ...
    public ResponseEntity<ErrorResponse> handleMissingParameter(MissingServletRequestParameterException exception, HttpServletRequest request) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body(new ErrorResponse(
                        "MISSING_PARAMETER",
                        messageResolver.resolve("error.MISSING_PARAMETER", new Object[]{exception.getParameterName()}, "Required parameter '" + exception.getParameterName() + "' is missing."),
                        requestId(request)
                ));
    }
    ...
    public ResponseEntity<ErrorResponse> handleMethodNotSupported(HttpRequestMethodNotSupportedException exception, HttpServletRequest request) {
        return ResponseEntity.status(HttpStatus.METHOD_NOT_ALLOWED)
                .body(new ErrorResponse(
                        "METHOD_NOT_ALLOWED",
                        messageResolver.resolve("error.METHOD_NOT_ALLOWED", null, exception.getMessage()),
                        requestId(request)
                ));
    }

    @ExceptionHandler(ObjectOptimisticLockingFailureException.class)
    public ResponseEntity<ErrorResponse> handleOptimisticLockingFailure(ObjectOptimisticLockingFailureException exception, HttpServletRequest request) {
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(new ErrorResponse(
                        "OPTIMISTIC_LOCK_CONFLICT",
                        messageResolver.resolve("error.OPTIMISTIC_LOCK_CONFLICT", null, "The resource was modified by another request. Please retry."),
                        requestId(request)
                ));
    }
    ...
    public ResponseEntity<ErrorResponse> handleUnexpectedException(Exception exception, HttpServletRequest request) {
        ...
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(new ErrorResponse(
                        ...
                        messageResolver.resolve("error.INTERNAL_SERVER_ERROR", null, "Unexpected server error"),
                        requestId(request)
                ));
    }
